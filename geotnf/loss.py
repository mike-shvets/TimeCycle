...
class TransformedGridLoss(nn.Module):
    ...
    def forward(self, theta, theta_GT):
        ...
        if self.geometric_model == 'affine':
            ...
            P_prime_GT = self.pointTnf.tpsPointTnf(
                theta_GT, P)
        else:
            raise AttributeError(
                'Unknown geometric model {}'.format(self.geometric_model))
...
class WeakInlierCountPool(nn.Module):
    def __init__(self,
        ...
        self.geometricTnf = GeometricTnf(
            ...
            out_h=int(h_matches / 3),
            out_w=int(w_matches / 3),
        ...
        # define identity mask tensor
        # (w,h are switched and will be permuted back later)
            ...
            if not (isinstance(dilation_filter, int) and dilation_filter == 0):
                for i in range(mask_id.shape[2]):
                    mask_id[:, :, i] = binary_dilation(
                        mask_id[:, :, i],
                        structure=dilation_filter).astype(mask_id.dtype)
        else:
            for i in range(mask_id.shape[2]):
    ...
    def forward(self, theta, matches, return_outliers=False):
        if isinstance(theta, Variable):  # handle normal batch transformations
            ...
            mask = self.geometricTnf(
                expand_dim(self.mask_id, 0, batch_size), theta)
            # print(mask.size())
            ...
            # handle multiple transformations per batch item,
            # batch is in list format (used for RANSAC)
            ...
            for b in range(batch_size):
                ...
                s = self.forward(
                    theta[b],
                    expand_dim(matches[b, :, :, :].unsqueeze(0), 0, sample_size))
                score.append(s)
        else:
            raise RuntimeError('theta is of wrong type ({})'.format(type(theta)))
        return score
