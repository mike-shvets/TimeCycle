...
for name in checkpoint_30.pth.tar
do
YOUR_DATASET_FOLDER=$(realpath './datasets')

echo $name
if [ ! -d "results/davis_results__$name/" ]; then

    python test_davis.py \
    "${YOUR_DATASET_FOLDER}/davis/DAVIS/vallist.txt" \
...
2>&1 | tee results/davis_results__${name}_converted/output.txt
